...
Loading:
 - Total displacement of -1 mm in the Y-direction at the top surface, ramped linearly over 10 timesteps
...
Modeling notes:
 - At each timestep, the global model is solved with the specified boundary conditions; 
   the resulting nodal displacements are interpolated to the boundary nodes of the local model, using the DPF
   interpolation operator. Those displacements are enforced as constraints to the local model, which is then solved 
   completing that timestep.
"""
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# We assign the instances to the local and global model, then use ``mapdl.cdread`` to load their geometry and mesh.
# Note the the .cdb files include named selections for the faces we want to apply the boundary conditions and the loads. 
# The function ``define_BCs`` defines the global model’s boundary conditions and applied loads. 
# The function ``Get_boundary`` is used to record the local model’s cut-boundary node coordinates as a dpf.field
# which will be later used in the DPF interpolator input

cwd = Path.cwd() # Get current working directory


mapdl_global = pool[0] # Global model
mapdl_global.cdread('db','global.cdb') # Load global model
mapdl_global.cwd(cwd/Path('Output/Global')) # Set directory of the global model

mapdl_local = pool[1] # Local model
mapdl_local.cdread('db','local.cdb') # Load local model
...
def Get_boundary(mapdl):
    ...
    boundary_coordinates = dpf.fields_factory.create_3d_vector_field(
        num_entities=len(node_id_subset),location='Nodal') # Define DPF field for DPF interpolator input
    
    nsel = '' 
    for nid in node_id_subset: # Iterate boundary nodes of the local model
        nsel+='nsel,A,NODE,,{}\n'.format(nid) # Add selection command for the node to the str (only for ploting)
...
def define_dpf_operators(nCores):
    ...
    for i in range(nCores):
        dataSources.set_domain_result_file_path(path=Path(f"./Output/Global/file{i}.rst"), key='rst', domain_id=i)
    
    global_model = dpf.Model(dataSources)
    global_disp_op = dpf.operators.result.displacement() # Define displacement result operator to read nodal displacements
    global_disp_op.inputs.data_sources.connect(dataSources) # Connect displacement result operator with the global model's results file
    disp_interpolator = dpf.operators.mapping.on_coordinates() # Define interpolator to interpolate the results inside the mesh elements with shape functions
    return global_model,global_disp_op,disp_interpolator

def initialize_dpf_interpolator(global_model,local_Bc_coords,disp_interpolator,):
    my_mesh = global_model.metadata.meshed_region # Global model's mesh
    disp_interpolator.inputs.coordinates.connect(local_Bc_coords) # Link interpolator inputs with the local model's boundary coordinates
    disp_interpolator.inputs.mesh.connect(my_mesh) # Link interpolator mesh with the global model's mesh
    
def interpolate_data(timestep):
    global_disp_op.inputs.time_scoping.connect([timestep]) # Specify timestep value to read results from
    global_disp = global_disp_op.outputs.fields_container.get_data() # Read global nodal displacements
    
    disp_interpolator.inputs.fields_container.connect(global_disp) # Link the interpolation data with the interpolator 
    local_disp = disp_interpolator.outputs.fields_container.get_data()[0] # Get displacements of the boundary nodes of the local model
    return local_disp
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# We solve the two models sequentially for each loading step. First the global model is run producing 
# a .rst results file. Then we extract the global displacements and use them to define 
# cut-boundary conditions for the local model (an input string command will be used for faster excecution time).
...
def define_cut_boundary_constraint_template(local_Bc_coords):
    ...
    local_nids = local_Bc_coords.scoping.ids # Get Node ID of boundary nodes of the local model
    template = ''
